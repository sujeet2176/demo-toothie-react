...
public class ControlPanelActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Photo&Video 720P
        b720p = Settings.getInstance(this).getParameterForPhoto720p();
        // 720P填充，VGA按比例拉伸
//        if (b720p)
...
//                mVideoView.setRotation180(!mVideoView.isRotation180());
                ...
                mRotationDegree += 90;
                mRotationDegree %= 360;
                mVideoView.setVideoRotation(mRotationDegree);
            }
        });
        ...
        mResolutionButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                // 录像时无法改变分辨率
                if (recording) {
                    Toast.makeText(ControlPanelActivity.this, R.string.control_panel_cannot_change_resolution, Toast.LENGTH_SHORT).show();
                    return;
                }
                showVideoResolution();
            }
...
//            Log.d(TAG, new String(data) + Arrays.toString(data));
    ...
    private void takePhoto(int num) {
        // Take a photo
        String photoFilePath = Utilities.getPhotoDirPath(getApplicationContext());
        String photoFileName = Utilities.getMediaFileName();
        try {
            if (b720p) {
                mVideoView.takePicture(photoFilePath, photoFileName, 1280, 720, num);
            } else {
    ...
    private void recordVideo() {
        if (recording) {
            ...
            mFreeSpaceMonitor = new FreeSpaceMonitor(getApplicationContext());
            if (mFreeSpaceMonitor.checkFreeSpace()) {
                String videoFilePath = Utilities.getVideoDirPath(getApplicationContext());
    ...
    private void showVideoResolution() {
        ...
        if (count > 0) {
            // 弹出选择分辨率
            ActionSheet.createBuilder(ControlPanelActivity.this, getSupportFragmentManager())
                    .setCancelButtonTitle("Cancel")
                    .setOtherButtonTitles(titles.toArray(new String[0]))
                    .setCancelableOnTouchOutside(true)
                    .setListener(ControlPanelActivity.this)
                    .show();
        }
    }
    ...
    private MultiplePermissionsListener allPermissionsListener = new MultiplePermissionsListener() {
        ...
        public void onPermissionsChecked(MultiplePermissionsReport report) {
            if (!report.areAllPermissionsGranted()) {
                ...
                Toast.makeText(ControlPanelActivity.this, msg, Toast.LENGTH_LONG).show();
            }
        }
