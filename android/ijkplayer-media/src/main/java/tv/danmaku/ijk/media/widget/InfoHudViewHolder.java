...
public class InfoHudViewHolder {
    ...
    private static String formattedTime(long ms) {
        long time = ms / 1000;
        if (time >= 3600) {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case MSG_UPDATE_HUD: {
                    ...
                    long rtpSpeed            = mp.getRtpSpeed();
                    long rtpByteCount        = mp.getRtpTrafficStatisticByteCount();
                    long connectedTime       = mp.getConnectedTime();
                    ...
                    setRowValue(R.string.rtp_speed, String.format(Locale.US, "%s", formatedSpeed(rtpSpeed, 1000)));
                    setRowValue(R.string.rtp_byte_count, String.format(Locale.US, "%s", formatedSize(rtpByteCount)));
                    setRowValue(R.string.connected_time, String.format(Locale.US, "%s", formattedTime(connectedTime)));
