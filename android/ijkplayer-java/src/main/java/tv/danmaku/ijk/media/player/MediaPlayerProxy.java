...
public class MediaPlayerProxy implements IMediaPlayer {
    ...
    @Override
    public void setOnInsertVideoListener(OnInsertVideoListener listener) {
        if (listener != null) {
            final OnInsertVideoListener finalListener = listener;
            mBackEndMediaPlayer.setOnInsertVideoListener(new OnInsertVideoListener() {
                @Override
                public void onInsertVideo(IMediaPlayer mp, int resultCode) {
                    finalListener.onInsertVideo(MediaPlayerProxy.this, resultCode);
                }
            });
        } else {
            mBackEndMediaPlayer.setOnInsertVideoListener(null);
        }
    }

    @Override
    public void setOnReceivedFrameListener(OnReceivedFrameListener listener) {
        if (listener != null) {
            final OnReceivedFrameListener finalListener = listener;
            mBackEndMediaPlayer.setOnReceivedFrameListener(new OnReceivedFrameListener() {
                @Override
                public void onReceivedFrame(IMediaPlayer mp, byte[] data, int width, int height, int pixelFormat) {
                    finalListener.onReceivedFrame(MediaPlayerProxy.this, data, width, height, pixelFormat);
                }
            });
        } else {
            mBackEndMediaPlayer.setOnReceivedFrameListener(null);
        }
    }

    @Override
    public void setOnReceivedOriginalDataListener(OnReceivedOriginalDataListener listener) {
        if (listener != null) {
            final OnReceivedOriginalDataListener finalListener = listener;
            mBackEndMediaPlayer.setOnReceivedOriginalDataListener(new OnReceivedOriginalDataListener() {
                @Override
                public void onReceivedOriginalData(IMediaPlayer mp, byte[] data, int width, int height, int pixelFormat, int videoId,int degree) {
                    finalListener.onReceivedOriginalData(MediaPlayerProxy.this, data, width, height, pixelFormat, videoId, degree);
                }
            });
        } else {
            mBackEndMediaPlayer.setOnReceivedOriginalDataListener(null);
        }
    }
